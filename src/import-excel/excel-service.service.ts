...
export class ExcelService {
    ...
    private readonly templateHeader = {
        columns: [
            { header: 'Username', key: 'username', width: 20, style: { font: { name: 'Arial' } } },
            {
                header: 'Password', key: 'password', width: 30, style: {
                    font: { name: 'Arial' }
                }, comment: COMMENT.PASSWORD
            },
            { header: 'Account type', key: 'type', width: 20, style: { font: { name: 'Arial' } }, comment: COMMENT.ACCOUNT_TYPE },
            { header: 'Role', key: 'role', width: 20, style: { font: { name: 'Arial' } }, comment: COMMENT.ROLE },
        ]
    };
    ...
    async exportTemplate() {
        ...
        for (const header of headers) {
            ...
            workSheet.getRow(1).getCell(header.key).style = { alignment: { horizontal: 'center' } };
    ...
    private validate(data, usernameSet: Set<string>) {
        ...
        if (type == "Local") {
            const validatePassword = this.manageUserLocalService.validatePassword(password, TYPE.LOCAL);
    ...
    private rowToJSON(row: any) {
        ...
        for (let i = 1; i < row.length; i++) {
            ...
            if (!field) continue;
            data[field] = (row[i] && row[i].result) ? row[i].result.toFixed(0) : (row[i] ? row[i].toString() : undefined);
        }
        return data;
    }
